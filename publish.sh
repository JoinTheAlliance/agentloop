...
# Read agentloop/__init__.py, find the line that starts with __version__ and replace with '__version__ = "${VERSION}"'
sed -i.bak "s/__version__.*/__version__ = \"${VERSION}\"/" agentloop/__init__.py
rm agentloop/__init__.py.bak
...
# Make sure these work, and stop the script if they error
# Upload to test repo
twine upload dist/agentloop-${VERSION}.tar.gz --repository-url https://test.pypi.org/legacy/ || { echo "Upload to test repo failed"; exit 1; }
pip install --index-url https://test.pypi.org/simple/ agentloop --user || { echo "Installation from test repo failed"; exit 1; }

# Final upload
twine upload dist/agentloop-${VERSION}.tar.gz || { echo "Final upload failed"; exit 1; }
pip install agentloop --user || { echo "Installation of agentloop failed"; exit 1; }

git add agentloop/__init__.py
git add setup.py
